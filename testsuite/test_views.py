...
from testsuite.factories import (
    DatasetFactory,
    LuDataBirdFactory,
    EtcDataBirdFactory,
    EtcBirdsEuFactory,
...
def test_summary_filter_form_reported_name(
    ...
    dataset = DatasetFactory()
    subject = LuDataBirdFactory(dataset=dataset)
    entry = EtcDataBirdFactory(
        dataset=dataset, assessment_speciesname=subject.speciesname
    )
    create_user("test_user")
    force_login(client, "test_user")
...
def test_report_get(
    ...
    response = client.get(
        url, params={"period": dataset.id, "country": entry.country_isocode}
    )
    assert response.status_code == 200
...
def test_eu_map_get(
    ...
    response = client.get(
        url, params={"suffix": "PR", "speciescode": entry.speciescode}
    )
    assert response.status_code == 200
