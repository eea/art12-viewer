...
class SpeciesMixin(object):
    ...
    def get_subjects(self, dataset):
        model = LuDataBird
        if dataset.id == 3:
            return [(entry.speciesname, entry.speciesname) for entry in (
            ...
            return [(entry.speciescode, entry.speciesname) for entry in (
                model.query.filter_by(dataset=dataset)
                .distinct()
    ...
    def get_reported_name(self, dataset, speciesname):
        ...
        return [(entry.speciescode, entry.reported_name) for entry in (
    ...
    def get_countries(self, dataset):
        if dataset.id == 2:
            return [(u"GR", u"GR")]
        return [(entry.country_isocode, entry.country) for entry in (
