...
SELECT DISTINCT A.speciesname
  FROM data_birds_check_list A
  WHERE A.speciescode = :value ;
"""
...
SELECT
  ...
  CASE
    WHEN A.population_minimum_size_bs IS null THEN null
    ELSE CONCAT_WS(
      ' ', A.population_minimum_size_bs,
      '-', A.population_maximum_size_bs,
      A.population_size_unit_bs)
  END AS breeding_population_size,
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        c.level2_name
      FROM (
        (data_bpressures_threats t
        INNER JOIN data_birds a ON (a.specieshash = t.specieshash))
        INNER JOIN data_birds_check_list b ON (
          a.country = b.country
      ...
      LEFT JOIN (
        ...
        LEFT JOIN lu_threats t_lu_threats
          ON q_lu_threats.level2_code = t_lu_threats.code
      ) c ON (c.code = t.pressurecode)
      WHERE
        a.speciescode = :value
        AND a.use_for_statistics = true
        AND b.spa_trigger = '1'
        AND NOT (t.pressurecode) IN ('U', 'X')
        AND Upper(t.rankingcode) = 'H'
   ) AS e
   GROUP BY e.level2_code, e.level2_name
 ) AS rs1
 INNER JOIN (
  SELECT
    Count(e.speciescode) AS pl2_tot,
    1 AS pl2_set
  FROM (
    SELECT
      DISTINCT a.country,
    ...
    FROM (
     (data_bpressures_threats t
     INNER JOIN data_birds a ON (a.specieshash = t.specieshash))
     INNER JOIN data_birds_check_list b
     ON (
       a.country = b.country
       AND a.speciescode = b.speciescode
       AND a.season = b.season
  ...
  LEFT JOIN lu_threats c ON (c.code = t.pressurecode)
  WHERE
    a.speciescode = :value
    AND a.use_for_statistics = true
    AND b.spa_trigger = '1'
  ...
  ) AS rs2 ON rs1.pl2_set = rs2.pl2_set
  ORDER BY Round(100 * rs1.pl2_num / rs2.pl2_tot) DESC
) AS rst
ORDER BY rst.pc DESC, rst.code ASC
LIMIT 10;
"""
...
SELECT
  ...
  IF(t.season = 'W', 'winter', 'breeding') AS wb,
    'YES' AS spa,
  IF((
    ( t.spa_population_min IS NULL AND t.spa_population_max IS NULL )
    ...
    100 * SQRT(IF(t.spa_population_min = 0, 1,
      IF(t.spa_population_min IS NULL, t.spa_population_max, t.spa_population_min)) *
...
FROM data_birds AS t
  INNER JOIN data_birds_check_list AS b
...
FROM (SELECT rs1.level2_code AS code,
      ...
      FROM (SELECT e.level2_code,
            ...
            FROM (SELECT DISTINCT a.country,
                                  ...
                                  c.level2_code,
                                  c.level2_name
                  FROM ((data_bmeasures m
                         INNER JOIN data_birds a
                         ...
                         LEFT JOIN (SELECT DISTINCT q_lu_measures.*,
                                    ...
                                    FROM (SELECT t.code,
                                          ...
                                          FROM lu_measures t)
                                          q_lu_measures
                                          LEFT JOIN lu_measures t_lu_measures
