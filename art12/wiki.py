...
class CommonSection(object):
    def get_req_args(self):
        return {
            ...
            for arg in ["subject", "period", "reported_name"]
        }
...
class AuditView(views.View):
    ...
    def get_wiki_changes_for_period_2018(self, rq, wikis):
        ...
        if len(wikis) > 1 and rq["reported_name"] == "":
            ...
            if rq["reported_name"]:
                wikis = wikis.filter(
                    self.section.wiki_cls.reported_name_code == rq["reported_name"]
                ).all()
            else:
                wikis = wikis.all()
...
wiki.add_url_rule(
    "/summary/datasheet/",
    view_func=WikiView.as_view("datasheet", section=DataSheetSection),
)

wiki.add_url_rule(
    "/summary/audittrail/",
    view_func=AuditView.as_view("audittrail", section=AuditTrailSection),
)
