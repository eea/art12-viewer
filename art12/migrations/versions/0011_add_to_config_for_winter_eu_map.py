...
def upgrade():
    ...
    op.add_column(
        "config",
        sa.Column(
            "eu_sensitive_species_map_winter_url", sa.String(length=255), nullable=True
        ),
    )
    op.add_column(
        "config",
        sa.Column("eu_species_map_breeding_url", sa.String(length=255), nullable=True),
    )
    op.add_column(
        "config",
...
def downgrade():
    op.add_column(
        "config",
        sa.Column("eu_species_map_url", mysql.VARCHAR(length=255), nullable=True),
    )
    op.add_column(
        "config",
        sa.Column(
            "eu_sensitive_species_map_url", mysql.VARCHAR(length=255), nullable=True
        ),
    )
